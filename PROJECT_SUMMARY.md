...
2. **Transcription Service** (`backend/services/transcription.py`)
   - DeepSeek Speech API (облачная транскрибация)
   - Word-level timestamps (нормализация ответа)
   - VAD фильтрация через DeepSeek

3. **Highlight Analyzer** (`backend/services/highlight_analyzer.py`)
   - DeepSeek Chat JSON-анализ
...
4. **Translation Service** (`backend/services/translation.py`)
   - DeepSeek Chat для перевода/локализации
   - Возвращает чистые субтитры + markup для Silero
   - Batch обработка
...
1. **install.sh** - автоматическая установка
   ...
   - Создание venv
   - Установка пакетов
   - Создание .env с DEEPSEEK_API_KEY
...
5. **docker-entrypoint.sh** - Docker entrypoint
   - Проверка наличия DEEPSEEK_API_KEY
   - Запуск приложения
...
### AI Models
- **DeepSeek Speech** - транскрипция
- **DeepSeek Chat** - анализ/перевод/субтитры
...
✅ Транскрипция - DeepSeek Speech  
✅ Анализ - DeepSeek Chat + 12 критериев  
✅ Перевод - DeepSeek Chat (адаптация)  
✅ Озвучка - Silero TTS  
...
HIGHLIGHT_CONCURRENT_REQUESTS=5
```

### Если не хватает VRAM
```env
SILERO_SPEAKER=aidar        # более лёгкий голос
HIGHLIGHT_CONCURRENT_REQUESTS=2
```
