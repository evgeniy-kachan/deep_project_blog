...
- **Облачная транскрипция** и извлечение смысла с помощью DeepSeek Speech API
- **AI-анализ** интересных моментов по 12 критериям через DeepSeek LLM
- **Автоматический перевод и адаптация** на русский язык в DeepSeek (с разметкой для TTS)
...
- **DeepSeek API** - транскрипция, анализ, перевод и генерация субтитров
...
- Python 3.10+
- Node.js 18+
- CUDA 11.8+ (для Silero TTS и рендеринга)
- FFmpeg
- DeepSeek API ключ (https://api.deepseek.com)
...
1. Зарегистрируйтесь в [DeepSeek](https://api.deepseek.com) и создайте API‑ключ.
2. Сохраните ключ в `.env` (см. ниже) или экспортируйте в окружение:
   ```bash
   export DEEPSEEK_API_KEY=sk-xxxx
   ```
3. (Опционально) Настройте альтернативные модели `deepseek-chat`, `deepseek-reasoner`, `deepseek-speech`.
...
DEEPSEEK_TRANSCRIBE_MODEL=deepseek-speech
...
### DeepSeek Chat/Reasoning модель

Отредактируйте `.env`:
```env
DEEPSEEK_CHAT_MODEL=deepseek-chat      # deepseek-reasoner, deepseek-coder и т.д.
```

### DeepSeek Speech модель

```env
DEEPSEEK_TRANSCRIBE_MODEL=deepseek-speech   # либо другая доступная модель
```
...
|------|------------------------|
| Транскрипция (DeepSeek Speech) | ~5-8 мин (зависит от сети) |
| Анализ LLM (DeepSeek Chat, 20 сегментов) | ~5-10 мин |
| Перевод + субтитры (DeepSeek Chat) | ~1-2 мин |
...
Она относится к Silero TTS или ffmpeg-фильтрам. Снизьте требования:
```python
# backend/services/tts.py
TTSService(max_chunk_chars=250, speaker="aidar")
```
или временно используйте CPU:
```bash
...
### DeepSeek отвечает ошибкой 401/429

- Проверьте `DEEPSEEK_API_KEY` и лимиты в личном кабинете.
- Убедитесь, что сервер имеет доступ к `https://api.deepseek.com`.
- При 429 уменьшите параллелизм через `HIGHLIGHT_CONCURRENT_REQUESTS` в `.env`.
